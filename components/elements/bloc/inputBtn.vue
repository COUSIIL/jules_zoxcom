<template>
  <div class="flex items-center justify-center h-8 m-2.5 overflow-visible rounded-full shadow-md" :class="{ 'animate-slideFromRight': isAnimating }" :style="{ backgroundColor: color }">
    <div class="relative w-full m-0.5">
      <input
        :type="type"
        :value="inputValue"
        @input="onInput"
        :required="required"
        :class="type === 'color' ? '' : 'bg-gradient-to-r from-whity to-whiby dark:from-darky dark:to-darkiw'"
        class="w-full h-7 px-3 py-3 text-base rounded-3xl outline-none"
        :style="type === 'color' ? { backgroundColor: inputValue } : {}"
      />
      <label class="absolute top-0 left-2.5 flex items-center justify-center h-4 px-1 text-xs text-whity -translate-y-1/2 rounded-full pointer-events-none transition-all duration-200 ease-in-out bg-zioly2">
        <div class="w-4 h-4 mx-0.5" v-html="resizedImg"></div>
        {{ t(placeHolder) }}
    ...
    <button v-if="svg" type="button" :style="{ color }" @click="onClicked" class="relative z-50 flex items-center justify-center w-8 h-8 mx-1.25 rounded-2xl cursor-pointer">
      <span class="w-3.5 h-3.5" v-html="resizedSvg" />
    </button>
  </div>
</template>
