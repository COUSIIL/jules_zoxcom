<template>
  <nav v-if="isMounted" class="sticky top-0 z-[5000] flex h-[50px] w-full items-center justify-between bg-whitly px-4 shadow-lg backdrop-blur-[10px] dark:bg-darkly dark:shadow-darky">
    <!-- Left side -->
    <NuxtLink to="/">
    ...
    <div class="flex items-center space-x-2">
      <NuxtLink v-if="isAuthenticated && isDeasy" to="/diniChat" class="flex h-10 w-10 cursor-pointer items-center justify-center">
        <img v-if="!isDark" :src="varqWhite" alt="Dini icon" class="h-6" />
        <img v-else :src="varqDark" alt="Dini white icon" class="h-6" />
      </NuxtLink>

      <button @click="toggleDarkMode" class="flex h-10 w-10 cursor-pointer items-center justify-center transition-all duration-500 ease-in-out">
        <svg
          v-if="isDark"
          key="dark"
          class="text-whity"
        ...
        <svg v-else key="light" class="text-darkly" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none">
      ...
      <button v-if="isAuthenticated" @click="sideBar" class="flex h-10 w-10 cursor-pointer items-center justify-center transition-all duration-500 ease-in-out">
