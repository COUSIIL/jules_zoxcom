  ...
  <div class="module-content">
    <div class="module-header">
      <div class="logo-container">
        <img src="../../public/anderson.png" alt="Anderson logo" class="logo-img" />
      </div>
    ...
    <div class="input-group">
      <h3>{{ t('key :') }}</h3>
      <input class="input" v-model="andersonKey" type="text" :placeholder="t('Enter Anderson API Key')">
    </div>

    <button v-if="!saving" class="btn2 save-btn" @click="applyAnderson" type="button">
      {{ t('save') }}
...
export default {
  ...
  methods: {
    async getAnderson () {
      this.loading = true;
      try {
        const response = await fetch('https://management.hoggari.com/backend/api.php?action=testAnderson', {
            method: 'GET'
        });
        ...
        if (textResponse.success) {
          this.andersonKey = textResponse.data.key;
    ...
    async applyAnderson() {
      ...
      const andersonModule = JSON.stringify({
        token: token,
        name: this.name,
        key: this.andersonKey || '',
        work: this.work ? 1 : 0,
      });
