  ...
  <div class="module-content">
    <div class="module-header">
      <div class="logo-container">
        <img src="https://management.hoggari.com/yalidine.png" alt="Yalidine logo" class="logo-img" />
      </div>
    ...
    <div class="input-group">
      <h3>{{ t('api_id :') }}</h3>
      <input class="input" v-model="yalidineId" type="text" :placeholder="t('Enter Yalidine API ID')">
    </div>

    <div class="input-group">
      <h3>{{ t('api_token :') }}</h3>
      <input class="input" v-model="yalidineToken" type="text" :placeholder="t('Enter Yalidine API Token')">
    </div>

    <button v-if="!saving" class="btn2 save-btn" @click="applyYalidine" type="button">
      {{ t('save') }}
  ...
  methods: {
    ...
    async applyYalidine() {
      ...
      const yalidineModule = JSON.stringify({
        token: token,
        name: this.name,
        api_id: this.yalidineId || '',
        api_token: this.yalidineToken || '',
        work: this.work ? 1 : 0,
      });
