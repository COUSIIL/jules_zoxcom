  ...
  <div class="module-content">
    <div class="module-header">
      <div class="logo-container">
        <img src="https://management.hoggari.com/guepex.png" alt="Guepex logo" class="logo-img" />
      </div>
      <h3 class="title">{{ t('guepex api key') }}</h3>
      <Toggle v-if="!loading" :toggle="work" @toggle="activator('gpx_module')"/>
    </div>

    <div class="input-group">
      <h3>{{ t('api_id :') }}</h3>
      <input class="input" v-model="guepexId" type="text" :placeholder="t('Enter Guepex API ID')">
    </div>

    <div class="input-group">
      <h3>{{ t('api_token :') }}</h3>
      <input class="input" v-model="guepexToken" type="text" :placeholder="t('Enter Guepex API Token')">
    </div>

    <button v-if="!saving" class="btn2 save-btn" @click="applyGuepex" type="button">
      {{ t('save') }}
...
export default {
  ...
  methods: {
    async getGpx () {
      ...
      try {
        ...
        if (textResponse.success) {
          this.guepexId = textResponse.api;
          this.guepexToken = textResponse.token;
    ...
    async applyGuepex() {
      ...
      const guepexModule = JSON.stringify({
        token: token,
        name: this.name,
        api_id: this.guepexId || '',
        api_token: this.guepexToken || '',
        work: this.work ? 1 : 0,
      });
      ...
      try {
        const response = await fetch('https://management.hoggari.com/backend/api.php?action=guepexModule', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: guepexModule,
        });
