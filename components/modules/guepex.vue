<template>
    <div style="width: 90%; display: flex; justify-content: space-between; align-items: center;">
        <li>
            <div style="width: 30px; height: 30px; border-radius: 8px; display: flex; justify-content: center; align-items: center;">
                <img src="https://management.hoggari.com/guepex.png" alt="">
            </div>
    ...
    <input class="input" v-model="guepexId" type="text">
    ...
    <input class="input" v-model="guepexToken" type="text">

    <button v-if="!saving" class="btn2" style="width: 50%;" @click="applyguepex" type="button">
      {{ t('save') }}
  ...
  methods: {
    ...
    async applyguepex() {
      ...
      const guepexModule = JSON.stringify({
        name: this.name,
        api_id: this.guepexId || '', // Si vide, envoie null
        api_token: this.guepexToken || '', // Si vide, envoie null
        work: job,
      });
