    ...
    <nav v-if="isVisible && isMounted" class="overlay">
      <div class="modal">
        ...
        <div v-else>
          ...
          @dragenter.prevent 
          @drop="handleCatDrop">
              <h2>{{ t('Showcase Category image') }}</h2>
              <div>
                <label for="imageUpload2" class="inputImg">
                  <span v-if="!descriptionImage">{{ t('Place square image here') }}</span>
                  <img 
                    v-else 
    ...
    methods: {
        ...
        const allowedTypes = ['image/webp', 'image/png', 'image/jpeg', 'image/jpg'];
          if (!allowedTypes.includes(file.type)) {
            console.error(this.t("rejected, Invalid image type. Only WebP, PNG, and JPG are allowed."))
            return;
          }

        if (!file) {
          console.error(this.t("No file selected."));
          return;
        }
          ...
          console.error(this.t("The selected file is not a valid Blob."));
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!file || !allowedTypes.includes(file.type)) {
          return reject(new Error(this.t('Invalid image type. Only WebP, PNG, and JPG are allowed.')));
        }
      ...
      async saveCat() {
        if (!this.descriptionBlob) {
          console.log(this.t('Please upload an image.'));
          return;
        }

        if (!(this.descriptionBlob instanceof Blob)) {
          console.log(this.t('The uploaded file is not a valid image.'));
          return;
        }

        try {
          console.log(this.t('Processing image, please wait...'));
          ...
          // Envoi au serveur
          console.log(this.t('Uploading image, please wait...'));
          ...
          if (!response.ok) {
            console.error(this.t('Error uploading image.'));
            return;
          }
          console.log(this.t('Response received, waiting for result...'));
          const result = await response.json();
