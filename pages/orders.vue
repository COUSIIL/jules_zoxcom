<template>
  ...
  <div v-if="!isMounted" class="fixed h-full w-full flex justify-center items-center self-center">
  ...
  <div v-else class="max-w-full flex flex-col items-center">
  ...
  <div v-else-if="isShipping">
    ...
    <div class="w-full mt-2.5 flex justify-center items-center">
      <Search v-model:searcher="searchValue" @search-submitted="search"></Search>
    </div>

    
    
    <div class="flex justify-between items-center w-full h-12 my-5 min-w-[200px] max-w-[800px]">
      <button class="flex justify-between items-center min-w-[80px] max-w-[150px] h-8 m-2.5 p-2 cursor-pointer bg-whitly rounded-md shadow-md text-darkly dark:bg-darkly dark:text-whitly" type="button" @click="toFilters">
        {{ t('filter') }}
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none" class="text-darkly dark:text-whitly ml-1.5">
      ...
      <div class="w-1/2 h-8 flex justify-between items-center min-w-[150px] max-w-[280px] bg-whitly rounded-md m-4 shadow-md dark:bg-darkly">
        <div class="relative inline-block text-left">
          <!-- Bouton pour ouvrir le menu -->
          <button @click="toggleMultyDropdown" class="flex justify-between items-center min-w-[100px] h-8 m-2.5 p-2 cursor-pointer text-darkly dark:text-whitly dark:bg-darkly">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none" class="text-darkly min-w-[25px] dark:text-whitly">
          ...
          <!-- Liste Dropdown -->
          <div v-if="isOpen3" class="absolute mt-2 w-52 bg-whitly dark:bg-darkly border rounded-lg shadow-lg z-50">
            <button v-for="(status, index2) in filteredStatus" :key="status.name" @click="updateSelectedOrder('status', status.value)" class="flex items-center w-48 px-4 py-1 text-left hover:bg-gorry m-1.5">
              <span v-if="status.svg" v-html="status.svg" class="w-6 h-6 mr-2"></span>
              <span>{{ status.name }}</span>
            </button>
          </div>
        </div>
        <button class="flex flex-col justify-center items-center h-12 w-full min-w-[24px] m-1.5 p-0.5 cursor-pointer overflow-hidden text-ellipsis whitespace-nowrap" @click="handleConfirm()">
        ...
        <div :class="selected ? 'flex justify-center items-center min-w-[12px] h-3 m-3 cursor-pointer bg-rady rounded-full' : 'flex justify-center items-center min-w-[14px] h-3.5 m-2.5 cursor-pointer border-2 border-darkly rounded-full dark:border-whitly'" @click="selectAll"></div>
      </div>
    </div>
    <div v-if="toFilter" class="h-auto flex flex-wrap justify-between items-center gap-1.5">
      <div class="inline-block text-left">
        ...
        <div 
          ...
          <button
            ...
            class="flex items-center w-48 px-4 py-1 text-left hover:bg-gorry m-1.5"
          >
            <span v-if="status.svg" v-html="status.svg" class="w-6 h-6 mr-2"></span>
    ...
    <template v-if="isListed">
      <div v-for="(id, index) in orderID" :key="id" class="w-full flex justify-center items-center flex-col">
        <div class="w-full flex justify-center items-center flex-col" v-if="isUpdating[index]">
          <Loader class="w-12 h-12" />
        </div>
        <div class="w-full flex justify-center items-center flex-col" v-else>
