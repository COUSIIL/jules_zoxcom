<template>
    <div v-if="isMounted" :style="{display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%'}">
                <form @submit.prevent="saveDelivery" :style="{width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}" v-if="!isUpdating || !isGetting">
                    <h2 class="order">Create new delivery country methode</h2>
                    <div class="boxContainer" style="min-width: 200px;">
                        ...
                        <input class="input" type="text" v-model="country" @blur="searchDelivery(country)" :disabled="true" required/>

            
                        <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <input class="input" type="text" v-model="city" :disabled="true" required/>
                        <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                            Local Administrative Unit name
                        ...
                        <input class="input" type="text" v-model="hall" :disabled="true" required/>
                        <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                            Adresse name
                        ...
                        <input class="input" type="text" v-model="adresse" :disabled="true" required/>
                    </div>
            
                    <div class="boxContainer" v-if="methode > 0" v-for="index in modal.length" :key="index">
                        <h3>
                            {{ index }}
                        </h3>
                        
                        <div class="list2">
                            <button v-if="isUps" class="btn1" style="min-width: 150px; flex-direction: column;" type="button" @click="modal[index - 1].name = 'ups'; setDelivery((index - 1), modal[index - 1].name)">
                                Ups Method
                            ...
                            <button class="btn1" style="min-width: 150px; flex-direction: column;" type="button" @click="modal[index - 1].name = 'default'; setDelivery((index - 1), modal[index - 1].name)">
                                Fast Method
                            ...
                            <button v-if="isYalidine" class="btn1" style="min-width: 150px; flex-direction: column;" type="button" @click="modal[index - 1].name = 'default'; setDelivery((index - 1), 'yalidine')">
                                Yalidine Method
                                <div style="display: flex; justify-content: center; align-items: center; background-color: #ff5555; padding: 5px; border-radius: 8px;">
                                    <img style="height: 40px" src="https://management.hoggari.com/yalidine.png"  alt="">
                                </div>
                        ...
                        <input class="input" type="text" required v-model="modal[index - 1].name" @blur="setDelivery((index - 1), modal[index - 1].name)"/>

                            <button v-if="modal[index - 1].more === false" class="btn2 flex flex-row" type="button" @click="modal[index - 1].more = !modal[index - 1].more">
                                <h3>
                                    Edit - Show more
                            ...
                            <button v-else class="btn2 flex flex-row" type="button" @click="modal[index - 1].more = !modal[index - 1].more">
                                <h3>
                                    Close - Show less
                        ...
                        <div class="list" style="flex-direction: column" v-if="modal[index - 1].more === true && modal[index - 1].options.length > 0" v-for="i in modal[index - 1].options.length" :key="index">
                            <div class="bg-whity dark:bg-darky" style="width: 90%; height: 60px; display: flex; justify-content: space-between; align-items: center;">
                                <div v-if="modal[index - 1].options[i - 1].more" style="min-width: 20px; cursor: pointer; margin-inline: 20px;" @click="modal[index - 1].options[i - 1].more = !modal[index - 1].options[i - 1].more">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                        <path d="M17.9998 15C17.9998 15 13.5809 9.00001 11.9998 9C10.4187 8.99999 5.99985 15 5.99985 15" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <div v-else style="min-width: 20px; cursor: pointer; margin-inline: 20px;" @click="modal[index - 1].options[i - 1].more = !modal[index - 1].options[i - 1].more">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                        <path d="M18 9.00005C18 9.00005 13.5811 15 12 15C10.4188 15 6 9 6 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                    
                                </div>
                                <button type="button" style="width: 100%; margin-inline: 20px; cursor: pointer; font-size: 2vh; font-weight: bold;" @click="modal[index - 1].options[i - 1].more = !modal[index - 1].options[i - 1].more">
                                ...
                                <button style="min-width: 20px; cursor: pointer; margin-inline: 20px;" @click="removeOptions(index, i )" type="button">
                            ...
                            <div v-if="modal[index - 1].options[i - 1].more" class="list" style="flex-direction: column">
                                <h3>
                                    {{ i }}
                                </h3>
                                <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <input class="input" type="text" required v-model="modal[index - 1].options[i - 1].cityName"/>
                                <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                                    Home Delivery Price
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                ...
                                <input class="input" type="text" required v-model="modal[index - 1].options[i - 1].homePrice"/>
                                <h4 style="width: 90%; max-width: 450px; display: flex; justify-content: space-between; align-items: center;">
                                    Stop-desk Delivery Price
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                ...
                                <input class="input" type="text" required v-model="modal[index - 1].options[i - 1].deskPrice"/>
                            ...
                            <button v-if="modal[index - 1].more" class="bg-whity dark:bg-darky" style="width: 90%; height: 60px; display: flex; justify-content: center; align-items: center; cursor: pointer;" type="button" @click="addOptions(index)">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                            ...
                            <button class="btn2" @click="removeMethode(index)" type="button">
                                <h2>Remove Methode {{index}}</h2>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                    <path d="M19.5 5.5L18.8803 15.5251C18.7219 18.0864 18.6428 19.3671 18.0008 20.2879C17.6833 20.7431 17.2747 21.1273 16.8007 21.416C15.8421 22 14.559 22 11.9927 22C9.42312 22 8.1383 22 7.17905 21.4149C6.7048 21.1257 6.296 20.7408 5.97868 20.2848C5.33688 19.3626 5.25945 18.0801 5.10461 15.5152L4.5 5.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                                    <path d="M3 5.5H21M16.0557 5.5L15.3731 4.09173C14.9196 3.15626 14.6928 2.68852 14.3017 2.39681C14.215 2.3321 14.1231 2.27454 14.027 2.2247C13.5939 2 13.0741 2 12.0345 2C10.9688 2 10.436 2 9.99568 2.23412C9.8981 2.28601 9.80498 2.3459 9.71729 2.41317C9.32164 2.7167 9.10063 3.20155 8.65861 4.17126L8.05292 5.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                                    <path d="M9.5 16.5L9.5 10.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                    ...
                    <div class="boxContainer">
                        <button class="btn2" type="button" @click="addMethode">
                            Add methode
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
                                <path d="M12 8V16M16 12L8 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12Z" stroke="currentColor" stroke-width="1.5" />
            ...
            <div v-else-if="isUpdating || isGetting" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
