<template>
  ...
  <div :style="{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}" v-if="isMounted">
    ...
    <Explorer v-if="isExplorer3" :show="isExplorer3" @confirm="getExplorerCatalog" @cancel="isExplorer3 = false" />
    <div class="boxProduct">
    ...
    <Explorer v-if="isExplorer4" :show="isExplorer4" @confirm="getExplorerModel" @cancel="isExplorer4 = false" />
    <div class="boxProduct">
...
const slug = ref('')
const catID = ref(0)
const catalog_index = ref(-1)
const catalog_image = ref('')
...
const addDetail = (qty, index, val) => {
  ...
  if(selectedDetail) {
    ...
    if(!exists) {
      selectedDetail = {color: color.value, size: size.value, qty: qty, index: index, colorName: colorName.value, catalog_image: catalogImage.value[newVal].previewImage, catalog_index: newVal}
      ...
      if (result.success) {
        ...
        for (let ii = 0; ii < result.data.length; ii++) {
          if (result.data[ii].id == productID.value) {
            ...
            cataActive.value = parseInt(result.data[ii].cata_active)
            ytbActive.value = parseInt(result.data[ii].ytb_active);
            slug.value = result.data[ii].slug;
            catID.value = result.data[ii].category
            ...
            for (let i = 0; i < result.data[ii].models.length; i++) {
              ...
              modal.value.push({ 
                ...
                imageBlob: '', 
                breakable: parseInt(result.data[ii].models[i].breakable),
...
async function saveProduct(models, desUrls, catalogUrls, productUrl) {
  ...
  try {
    const postProduct = JSON.stringify({
      ...
      colors: colors.value,
      sizes: sizes.value,
      ytbActive: parseInt(ytbActive.value),
    ...
    if(result.success) {
      ...
      isMessage.value = true;
      message.value = result.message
      navigateTo('/products', { redirectCode: 302 })
    } else {
      uploading.value = false;
      isMessage.value = true;
      message.value = result.message
