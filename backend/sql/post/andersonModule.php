        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($update_query) {
                ...
                if ($update_query->execute()) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'anderson updated successfully.'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Error updating anderson: ' . $update_query->error
                    ]);
                }
                $update_query->close();
            }
            ...
            if ($insert_query) {
                ...
                if ($insert_query->execute()) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'anderson added successfully.'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Error inserting anderson: ' . $insert_query->error
                    ]);
                }
                $insert_query->close();
            }
