...
// Création de la table (correction de la virgule après view INT)
$tableSQL = <<<SQL
CREATE TABLE IF NOT EXISTS viewPage (
...
$check = $mysqli->prepare("SELECT id, view FROM viewPage WHERE product_id = ?");
...
if ($result->num_rows > 0) {
    ...
    $update = $mysqli->prepare("UPDATE viewPage SET view = view + 1 WHERE product_id = ?");
    $update->bind_param("i", $product_id);
    $success = $update->execute();
    $update->close();
} else {
    // Nouveau : on insère la ligne
    $insert = $mysqli->prepare("INSERT INTO viewPage (product_id) VALUES (?)");
...
echo json_encode([
    'success' => $success,
    'message' => $success ? 'View updated successfully' : 'Failed to update view'
]);
?>
