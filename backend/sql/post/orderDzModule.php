<?php
// backend/sql/post/orderDzModule.php
...
$table_check_query = "SHOW TABLES LIKE 'orderdz_module'";
$table_check_result = $mysqli->query($table_check_query);

if ($table_check_result->num_rows == 0) {
    $create_table_query = "CREATE TABLE `orderdz_module` (
...
if (isset($data['name'], $data['key'])) {
    ...
    $check_query = $mysqli->prepare("SELECT id FROM `orderdz_module` WHERE name = ?");
    if ($check_query) {
        ...
        if ($check_result->num_rows > 0) {
            $update_query = $mysqli->prepare("UPDATE `orderdz_module` SET data_time = ?, work = ?, `key` = ? WHERE name = ?");
            if ($update_query) {
                $update_query->bind_param("siss", $data_time, $work, $key, $name);
                if ($update_query->execute()) {
                    echo json_encode(['success' => true, 'message' => 'orderDz updated successfully.']);
                } else {
                    echo json_encode(['success' => false, 'message' => 'Error updating orderDz: ' . $update_query->error]);
                }
                $update_query->close();
            }
        } else {
            $insert_query = $mysqli->prepare("INSERT INTO `orderdz_module` (data_time, work, name, `key`) VALUES (?, ?, ?, ?)");
            if ($insert_query) {
                $insert_query->bind_param("siss", $data_time, $work, $name, $key);
                if ($insert_query->execute()) {
                    echo json_encode(['success' => true, 'message' => 'orderDz added successfully.']);
                } else {
                    echo json_encode(['success' => false, 'message' => 'Error inserting orderDz: ' . $insert_query->error]);
                }
                $insert_query->close();
            }
        }
