        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($update_query) {
                ...
                if ($update_query->execute()) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'orderDz updated successfully.'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Error updating orderDz: ' . $update_query->error
                    ]);
                }
                $update_query->close();
            }
            ...
            if ($insert_query) {
                ...
                if ($insert_query->execute()) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'orderDz added successfully.'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Error inserting orderDz: ' . $insert_query->error
                    ]);
                }
                $insert_query->close();
            }
